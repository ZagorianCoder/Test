				  ...
				  unsigned int flags)
{
	...
	printk(KERN_INFO "vfat_lookup is now running");
		       ...
		       bool excl)
{
	...
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	printk(KERN_INFO "vfat_create is being executed");
	ts = current_time(dir);
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	printk(KERN_INFO "vfat_rmdir reached after dir_empty");
	if (err)
		goto out;
	err = vfat_find(dir, &dentry->d_name, &sinfo);
	if (err)
...
static int vfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	printk(KERN_INFO "reached vfat_unlink");
	err = vfat_find(dir, &dentry->d_name, &sinfo);
	if (err)
		goto out;
...
static int vfat_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	printk(KERN_INFO "reached vfat_mkdir after add entry");
	if (err)
		goto out_free;
	dir->i_version++;
	inc_nlink(dir);
		       ...
		       unsigned int flags)
{
	...
	printk(KERN_INFO "Vfat rename reached after find");
	if (err)
		goto out;
