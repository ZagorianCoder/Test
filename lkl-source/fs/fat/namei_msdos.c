				   ...
				   unsigned int flags)
{
	...
	printk(KERN_INFO "msdos_lookup reached");
	switch (err) {
	case -ENOENT:
		inode = NULL;
		break;
			...
			bool excl)
{
	...
	printk(KERN_INFO "msdos_create is now being executed");
	if (err)
		goto out;
...
static int msdos_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	printk(KERN_INFO "msdos_rmdir reached, fat_dir_empty completed");
	if (err)
		goto out;
	err = msdos_find(dir, dentry->d_name.name, dentry->d_name.len, &sinfo);
	if (err)
...
static int msdos_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	printk(KERN_INFO "entered in msdos_mkdir");
	err = msdos_format_name(dentry->d_name.name, dentry->d_name.len,
				msdos_name, &MSDOS_SB(sb)->options);
	if (err)
		goto out;
...
static int msdos_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	mutex_lock(&MSDOS_SB(sb)->s_lock);
	err = msdos_find(dir, dentry->d_name.name, dentry->d_name.len, &sinfo);
	printk(KERN_INFO "msdos_unlink is now being executed");
	if (err)
		goto out;
			...
			unsigned int flags)
{
	...
	printk(KERN_INFO "msdos_format_name completed in msdos_rename");
	if (err)
		goto out;
